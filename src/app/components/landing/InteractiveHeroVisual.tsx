...
import { 
  Check, ArrowUpRight, AlertCircle, Clock, FileText, CheckCircle2 
} from 'lucide-react';
...
function DashboardContent() {
  // Static stats for the dashboard preview
  const stats = {
    total: 167,
    compliant: 142,
    atRisk: 18,
  ...
  return (
    <div className="flex h-[600px] md:h-[700px] bg-slate-50">
      
      {/* SIDEBAR */}
      <div className="hidden md:flex w-64 flex-col bg-white border-r border-slate-200">
        <div className="h-16 flex items-center px-6 border-b border-slate-100">
           {/* LOGO */}
           <svg width="110" height="24" viewBox="0 0 800 168" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-slate-900">
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-xs px-2 py-0.5 bg-slate-100 rounded-full text-slate-600 font-medium">Live</span>
              </div>
...
function NavItem({ icon: Icon, label, active = false }: any) {
  return (
    <div className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-slate-100 text-[#3A4F6A]' : 'text-slate-500'}`}>
...
function VendorRow({ name, type, status, date }: any) {
  ...
  if (status === "Missing") statusColor = "bg-slate-100 text-slate-600 border-slate-200";

  return (
    <div className="px-6 py-4 flex items-center justify-between group hover:bg-slate-50 transition-colors relative overflow-hidden">
       <div className="flex items-center gap-4 relative z-10">
         <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 ${status === "Verified" ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-500"}`}>
       ...
       <div className="relative z-10 text-right">
         <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${statusColor} transition-colors duration-500`}>
           {status}
         </div>
         <div className="text-[10px] text-slate-400 mt-1 font-medium">{date}</div>
       </div>
    </div>
  );
}